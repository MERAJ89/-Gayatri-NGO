...
/* =====================================================
   PROGRAMME PAGE FUNCTIONS
   ===================================================== */

// Change quantity of needy people
function changeQuantity(button, change) {
    // Find the parent card
    const card = button.closest('.programme-card');
    const quantityDisplay = card.querySelector('.quantity-display');
    const basePriceElement = card.querySelector('[data-base-price]');
    ...
    const basePrice = parseInt(basePriceElement.getAttribute('data-base-price'));
    const totalPrice = basePrice * quantity;
    
    totalPriceDisplay.textContent = '₹' + totalPrice;
    totalPriceDisplay.setAttribute('data-total-price', totalPrice);
}

// Handle donate button clicks from programme page
function handleDonate(button, baseAmount, programme) {
    // Find the parent card to get the total amount
    const card = button.closest('.programme-card');
    const totalPriceDisplay = card.querySelector('.total-price-display');
    const quantityDisplay = card.querySelector('.quantity-display');
    
    // Get the total amount (either from data attribute or calculate it)
    let totalAmount = baseAmount;
    if (totalPriceDisplay) {
        const dataTotalPrice = totalPriceDisplay.getAttribute('data-total-price');
        if (dataTotalPrice) {
            totalAmount = parseInt(dataTotalPrice);
        } else {
    ...
    // Store the programme data in sessionStorage
    sessionStorage.setItem('donationAmount', totalAmount);
    sessionStorage.setItem('programmeName', programme);
    
    // Redirect to donate page
    window.location.href = 'donate.html';
}

// When donate.html page loads, populate with programme data if available
document.addEventListener('DOMContentLoaded', function() {
    const programmeName = sessionStorage.getItem('programmeName');
    const donationAmount = sessionStorage.getItem('donationAmount');
    
    if (programmeName && donationAmount) {
        const amountInput = document.getElementById('donationAmount');
        if (amountInput) {
            amountInput.value = donationAmount;
        }
        
        // Add programme info to form (optional)
        const formTitle = document.querySelector('.donation-container h2');
        if (formTitle) {
            formTitle.textContent = `Support: ${programmeName}`;
        }
    }
});
